...
/** Layout helper that shows the animated loader only on the home route and immediately passes through on all other routes. */
...
export function SplashGate({ children }: { children: React.ReactNode }) {
  ...
  // The splash animation is intentionally a tiny state machine: show loader, swipe it away, then stay done.
  const [phase, setPhase] = useState<Phase>("loading");

  useLayoutEffect(() => {
    // Non-home routes should never flash the loader during navigation or refresh.
  ...
  useEffect(() => {
    ...
    // Step 1 starts the upward swipe, step 2 fully removes the overlay from rendering.
